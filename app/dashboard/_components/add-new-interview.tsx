...
function AddNewInterview() {
    ...
    const onSubmit=async(e: React.FormEvent<HTMLFormElement>)=>{
        ...
        const InputPrompt="Job position: "+jobPosition+", Job Description: "+jobDesc+", Years of Experience : "+jobExperience+" , Depends on Job Position, Job Description & Years of Experience give us "+process.env.NEXT_PUBLIC_INTERVIEW_QUESTION_COUNT+" Interview question along with Answer in JSON format,Give ONLY an array of ${process.env.NEXT_PUBLIC_INTERVIEW_QUESTION_COUNT} interview questions and answers in pure JSON format like this:[{ "+"question"+": "+"..."+","+ "answer"+":"+"..."+" },...]No additional text, markdown or formatting. Only JSON.`;"

        const result=await chatSession.sendMessage(InputPrompt);
        const responseText = await result.response.text();
        const jsonMatch = responseText.match(/```(?:json)?\s*([\s\S]*?)\s*```/i);
        const MockJsonResp=jsonMatch ? jsonMatch[1].trim() : responseText.trim();
  ...
  return (
    <div>
        ...
        <DialogContent className="max-w-2xl  ">
            <DialogHeader >
            <DialogTitle className="text-2xl  text-white" >Tell us more about your job interviwing</DialogTitle>
            <DialogDescription asChild>
                <div>
                <form onSubmit={onSubmit}>
                <div>
                   <h2 className=''>Add Details about yout job position/role, Job description and years of experience</h2>
                ...
                <div className='flex gap-5 justify-end'>
                    <Button type="button" className='cursor-pointer' variant="ghost" onClick={()=>setOpenDailog(false)}>Cancel</Button>
